...
4. Next, read [this article](http://javascript.info/function-basics) from Javascript.info. We've mentioned this before, but JavaScript has changed a bit over the years and functions have recently received some innovation. This article covers one of the more useful new abilities: 'default parameters'. \(NOTE: In the subsection labelled 'Parameters', the article specifies that a "function always gets a copy of the value"- This wording is tricky! In the provided example the function in question is receiving a primitive data type (String) as an argument, and in that case the parameter 'from' does indeed copy the value of the primitive. However, if the function were to receive an object as an argument (which you will learn about later), the reference to the object is copied to the local variable 'from'! In this case it is said that the **value** of the variable which refers to the object is a **memory address** which is a reference to the object's location in memory. This concept causes confusion and will be covered later in the course! Furthermore, The last "task" at the end of this lesson uses loops, which you will learn about in the next lesson.  Don't worry about that one.\)  
